...
local itemsToSell = {
	...
	{ itemName = "durable exercise axe", clientId = 35280, buy = 2200000, count = 10000 },
	{ itemName = "durable exercise bow", clientId = 35282, buy = 2200000, count = 10000 },
	{ itemName = "durable exercise club", clientId = 35281, buy = 2200000, count = 10000 },
	{ itemName = "durable exercise rod", clientId = 35283, buy = 2200000, count = 10000 },
	{ itemName = "durable exercise wand", clientId = 35284, buy = 2200000, count = 10000 },
	{ itemName = "durable exercise sword", clientId = 35279, buy = 2200000, count = 10000 },
	{ itemName = "lasting exercise wand", clientId = 35290, buy = 7560000, count = 34400 },
	{ itemName = "lasting exercise rod", clientId = 35289, buy = 7560000, count = 34400 },
	...
	{ itemName = "lasting exercise axe", clientId = 35286, buy = 7560000, count = 34400 },
	{ itemName = "lasting exercise bow", clientId = 35288, buy = 7560000, count = 34400 },
	{ itemName = "lasting exercise club", clientId = 35287, buy = 7560000, count = 34400 },
	{ itemName = "lasting exercise sword", clientId = 35285, buy = 7560000, count = 34400 },
	...
	{ itemName = "pair of soulstalkers", clientId = 34098, sell = 10000000 },
	...
	{ itemName = "charged alicorn ring", clientId = 39180, sell = 10000000 },
	...
	{ itemName = "doll", clientId = 2991, buy = 25000000 },
	{ itemName = "exalted core", clientId = 37110, buy = 2000000 },
	{ itemName = "silver token", clientId = 22516, buy = 2000000 },
	{ itemName = "sliver", clientId = 37109, buy = 1000000 },
	{ itemName = "sliver", clientId = 37109, sell = 500000 },
	{ itemName = "sliver", clientId = 37109, sell = 500000 },
	...
	{ itemName = "might ring", clientId = 3048, buy = 2000, sell = 250, count = 20 },
	...
	{ itemName = "ring of red plasma", clientId = 23533, sell = 6000 },
	{ itemName = "ring of green plasma", clientId = 23531, sell = 6000 },
	{ itemName = "ring of blue plasma", clientId = 23529, sell = 6000 },
...
local cooldownTime = 5 -- Tempo de cooldown em segundos (5 minutos)
local batchSize = 50 -- Quantidade de itens vendidos por vez
...
function sellItemAction.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    local lastUsedTime = player:getStorageValue(sellItemID) -- Obtém o último tempo de uso registrado

    if lastUsedTime == -1 or currentTime - lastUsedTime >= cooldownTime then
        -- Se o último uso foi registrado há mais de 10 minutos ou nunca foi usado, permita o uso
        if item:getId() == sellItemID then
            local itemsSold = 0
            local totalSellValue = 0
            local inbox = player:getSlotItem(CONST_SLOT_STORE_INBOX)
            local itemInbox = inbox:getItems(true)

            for _, itemConfig in ipairs(itemsToSell) do
                local itemIDToSell = itemConfig.clientId
                local sellPrice = itemConfig.sell

                if sellPrice then
                    if itemInbox then
                        for i = 1, #itemInbox do
                            local itemId = itemInbox[i]:getId()
                            if itemId == itemIDToSell then
                                local sellValue = 0
                                local itemCount = itemInbox[i]:getCount()
                                local itemsToSellNow = math.min(itemCount, batchSize)
                                sellValue = sellPrice * itemsToSellNow
                                itemInbox[i]:remove(itemsToSellNow)
                                itemsSold = itemsSold + itemsToSellNow
            ...
            if itemsSold > 0 then
                if player:getFreeCapacity() >= getMoneyWeight(totalSellValue) then
                    player:addMoney(totalSellValue)
                    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("Você vendeu %d itens e recebeu %d gold.", itemsSold, totalSellValue))
                else
                    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Você não possui capacidade suficiente para receber o ouro.")
                end
            else
                player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Você não possui nenhum item para vender.")
            end

            player:setStorageValue(sellItemID, currentTime) -- Registra o novo tempo de uso
            return true
        end
    else
        ...
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("Aguarde %d minutos e %d segundos antes de usar novamente.", minutes, seconds))
    end

    return false
end

sellItemAction:id(sellItemID)
